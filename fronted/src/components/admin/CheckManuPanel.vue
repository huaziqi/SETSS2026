...
const emit = defineEmits<{
  ...
  (e: "view-detail", id: number): void; // 🔥 新增事件
}>();
...
const loadManuscripts = async () => {
  loading.value = true;
  try {
    await fetchData("/api/manuscript/admin/check");
    ...
    if (response && response.code === 200) {
      ...
      manuscripts.value = rawList
        ...
        .map((item: any) => ({
          ...
          status: item.status || "AwaitingChecking",
        }));
    } else {
...
// 🔥 修改：发射事件而不是路由跳转
const handleCheck = (item: ManuscriptItem) => {
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-for="item in filteredManuscripts" :key="item.manuscriptId">
            ...
            <td>
              <span class="status-badge pending">
                {{ item.status || "AwaitingChecking" }}
              </span>
            </td>

            <td class="action-col">
              <button class="check-btn" @click="handleCheck(item)">
                Check
              </button>
            </td>
          </tr>
          ...
          <tr v-if="filteredManuscripts.length === 0 && !loading">
            <td colspan="6" class="empty">No manuscripts awaiting check.</td>
          </tr>
